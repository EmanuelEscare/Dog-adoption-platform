        ...
        <div class="row">
            ...
            <div class="col-lg-9">
                <div class="container shadow" style="border-radius: 1rem;">
                    <br>
                    <div>
                    <form action="{{route('editarPerro')}}" method="POST">
                        @csrf
                    <br>
                    <h2>Modificar datos de "{{$perro->nombre}}"</h2>
                    <div style="min-height: 70px; margin-top: 13px;">
                        @if (Session::has('mensaje'))
                        <div class="alert alert-success  alert-dismissible fade show" role="alert">
                             Se modifico con exito los datos del perro.
...
{{--  --}}
                        ...
                        <div style="color: red; font-size: .9rem;">Por favor, introduzca el nombre del perro.</div>
                        @endif
                        <input type="text" name="nombre" class="form-control" @if(old('nombre')) value="{{old('nombre')}}" @else value="{{$perro->nombre}}" @endif placeholder="">
                        <br>
                        <p>Raza del perro</p>
                        ...
                        <br>
                        <p>Edad del perro</p>
                        <input type="number" min="0" max="20" @if(old('edad')) value="{{old('edad')}}" @else value="{{$perro->edad}}" @endif name="edad" class="form-control" placeholder="">
                        <br>
                        <p>Peso del perro (kg)</p>
                        <input type="number" min="0" max="100" @if(old('peso')) value="{{old('peso')}}" @else value="{{$perro->peso}}" @endif name="peso" class="form-control" placeholder="">
                        <br>
                        <p>Sexo del perro</p>
                        <select class="form-select" name="sexo">
                            <option value="{{$perro->sexo}}">@if($perro->sexo == 0) Macho @else Hembra @endif</option>
                            <option value="0" @if(old('sexo') == '0') selected @endif>Macho</option>
                            <option value="1" @if(old('sexo') == '1') selected @endif>Hembra</option>
                        </select>
                        <br>
                        <p>Descripción del perro.</p>
                        <textarea class="form-control" rows="10" name="descripcion">
                            @if(old('descripcion')) {{old('descripcion')}} @else {{$perro->descripcion}} @endif
                        </textarea>
                        <br>
                        <p>Disponibilidad del perro</p>
                        <select class="form-select" name="disponibilidad">
                            <option value="{{$perro->disponibilidad}}">@if($perro->disponibilidad == 0) Disponible @else No disponible @endif </option>
                            <option value="0" @if(old('disponibilidad') == '0') selected @endif>Disponible </option>
                            <option value="1" @if(old('disponibilidad') == '1') selected @endif>No disponible </option>
                        </select>
                        <br>
...
<br>
                ...
                {{--  --}}
                <div class="container bg-warning" style="border-radius: 1rem;">
                <br>
                <h3>Gestion de fotos</h3>
                <br>
                <form action="{{route('agregarFotoPerro')}}" method="POST" enctype="multipart/form-data">
                   <input type="hidden" name="idperro" value="{{$perro->id}}"> 
                   @csrf
                <div class="input-group input-group-lg mb-3">
                    <input required type="file" accept="image/*" class="form-control" name="foto" placeholder="">
                    <button class="btn boton" style="border: rgb(52, 141, 0) 1px solid; color: white;" type="submit">
              ...
              <br>
                ...
                <div id="carouselExampleCaptions" class="carousel" data-bs-ride="carousel">
                    ...
                    <button type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide-to="{{$stoper}}" class="@if($stoper == 0)active @endif" aria-current="true" aria-label="Slide 1"></button>
                    @php $stoper = $stoper+1; @endphp
                    @endforeach
                    ...
                    @php $stoper = 0; @endphp
                    @foreach ($fotos as $foto)
                    <div class="carousel-item @if($stoper == 0)active @endif" data-bs-interval="10000000">
                        @php $stoper = $stoper+1; @endphp
                        <img style="max-width: 300px; height: 300px; margin: auto;" src="{{ url('getfile/' .$foto->foto_url )}}" class="d-block w-100" alt="...">
                        <div class="carousel-caption d-md-block">
                                <a href="{{route('eliminarFotoPerro',$foto->id)}}" style="min-width: 200px;" class="btn btn-danger btn-lg"><i class="fa-solid fa-trash-can"></i></a>
                        </div>
                    </div>
                    @endforeach
